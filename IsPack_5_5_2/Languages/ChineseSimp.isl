...
SetupFileMissing=安装目录中的文件 %1 丢失。请修正此问题或获取一个新的程序副本。
SetupFileCorrupt=安装文件被破坏。请获取一个新的程序副本。
SetupFileCorruptOrWrongVer=安装文件被破坏，或者与此安装程序的版本不兼容。请修正此问题或获取新的程序副本。
NotOnThisPlatform=此程序将无法运行于 %1。
OnlyOnThisPlatform=此程序必须运行于 %1。
OnlyOnTheseArchitectures=此程序只能在为下列处理器结构设计的 Windows 版本中进行安装:%n%n%1
MissingWOW64APIs=你正在运行的 Windows 版本不包含执行 64 位安装程序所需的功能。要修正此问题，请安装 Service Pack %1。
WinVersionTooLowError=此程序需要 %1 版本 %2 或更高版本。
WinVersionTooHighError=此程序无法安装在 %1 版本 %2 或更高版本。
AdminPrivilegesRequired=您必须以管理员身份登录。
PowerUserPrivilegesRequired=您必须以管理员身份或有权限的用户组身份登录安装此程序。
SetupAppRunningError=安装程序检测到 %1 当前正在运行。%n%n请先关闭所有运行的窗口，然后单击“确定”继续，或按“取消”退出。
UninstallAppRunningError=卸载程序检测到 %1 当前正在运行。%n%n请先关闭所有运行的窗口，然后单击“确定”继续，或按“取消”退出。
...
ExitSetupMessage=安装程序未完成安装。如果您现在退出，您的程序将无法被安装。%n%n您可以以后再运行安装程序完成安装。%n%n确认退出安装程序吗？
...
LicenseLabel3=请仔细阅读下列许可协议。您在继续安装前必须同意这些协议条款才能继续安装。
...
ToUNCPathname=安装程序无法安装到一个 UNC 路径名。如果您正在尝试安装到网络，您需要映射一个网络驱动器。
InvalidPath=您必须输入一个带驱动器卷标的完整路径，例如:%n%nC:\APP%n%n或下列形式的 UNC 路径:%n%n\\server\share
InvalidDrive=您选定的驱动器或 UNC 共享不存在或无法访问。请选选择其它位置。
...
DirExists=文件夹:%n%n%1%n%n已经存在。您一定要安装到此文件夹中吗？
DirDoesntExistTitle=文件夹不存在
DirDoesntExist=文件夹:%n%n%1%n%n不存在。您想要创建此文件夹吗？

; *** “选择组件”向导页
WizardSelectComponents=选择组件
SelectComponentsDesc=您想安装程序的哪些组件？
...
NoUninstallWarning=安装程序侦测到您的电脑已安装下列组件。:%n%n%1%n%n取消选定这些组件将无法卸载它们。%n%n您一定要继续吗？
ComponentSize1=%1 KB
ComponentSize2=%1 MB
ComponentsDiskSpaceMBLabel=当前所选的组件至少需要 [mb] MB 的磁盘空间。
...
ReadyLabel2a=单击“安装”继续此安装程序。如果您想要回滚或改变设置，请单击“上一步”。
...
PreviousInstallNotCompleted=先前程序的安装/卸载未完成。您需要重新启动您的电脑才能完成安装。%n%n在重新启动电脑后，再完成 [name] 的安装。
...
SelectDiskLabel2=请插入磁盘 %1 并单击“确定”。%n%n如果此磁盘中的文件无法在不同于下列显示的文件夹中找到，输入正确的路径或单击“浏览”。
PathLabel=路径(&P):
FileNotInDir2=文件“%1”无法在“%2”定位。请插入正确的磁盘或选择其它文件夹。
...
; *** 文件复制错误
FileAbortRetryIgnore=单击“重试”进行重试，单击“忽略”跳过此文件 (不推荐)，或单击“中断”取消安装。
FileAbortRetryIgnore2=单击“重试”进行重试，单击“忽略”继续处理 (不推荐)，或单击“中断”取消安装。
SourceIsCorrupted=源文件被破坏
SourceDoesntExist=源文件“%1”不存在
ExistingFileReadOnly=现有的文件标记为只读。%n%n单击“重试”删除只读属性后再试，单击“忽略”跳过此文件，或单击“取消”退出安装。
...
ErrorRestartingComputer=安装程序无法重新启动电脑，请手动重启。

; *** 卸载消息
UninstallNotFound=文件“%1”不存在。无法卸载。
UninstallOpenError=文件“%1”无法打开。无法卸载
UninstallUnsupportedVer=卸载日志文件“%1”有未被此版本的卸载器承认的格式。无法卸载
UninstallUnknownEntry=在卸载日志中遇到一个未知的条目 (%1)
ConfirmUninstall=您确认想要完全删除 %1 及它的所有组件吗？
UninstallOnlyOnWin64=此安装程序只能在 64 位 Windows 中进行卸载。
OnlyAdminCanUninstall=此安装的程序只能是有管理员权限的用户才能卸载。
UninstallStatusLabel=正在从您的电脑中删除 %1，请等待。
UninstalledAll=%1 已顺利地从您的电脑中删除。
UninstalledMost=%1 卸载完成。%n%n有一些内容无法被删除。您可以手工删除它们。
UninstalledAndNeedsRestart=要完成 %1 的卸载，您的电脑必须重新启动。%n%n您现在想重新启动电脑吗？
UninstallDataCorrupted=“%1”文件被破坏，无法卸载

; *** 卸载状态消息
ConfirmDeleteSharedFileTitle=删除共享文件吗？
ConfirmDeleteSharedFile2=系统中包含的下列共享文件已经不被其它程序使用。您想要卸载程序删除这些共享文件吗？%n%n如果这些文件被删除，但还有程序正在使用这些文件，这些程序可能无法正确执行。如果您无法确定，选择“否”。把这些文件保留在系统中以免引起问题。
